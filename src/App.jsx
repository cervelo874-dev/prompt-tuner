                ...
                <div className="flex-1 overflow-hidden">
                    <ChatInterface
                        ...
                        onApplyPrompt={(prompt) => {
                            // テンプレートとして読み込む
                            // 変数を抽出して適用することも可能だが、今回はシンプルにテキスト全体をテンプレートとして設定
                            // 既存の変数は維持しつつ、新しいテンプレートを適用
                            updateVariable('prompt', prompt);
                            // または loadTemplate({ template: prompt, variables: {} }) 
                            // ここではシンプルに updateVariable を使うか、
                            // usePromptTemplate に setTemplateString のようなメソッドを追加するのがベストだが、
                            // 既存の loadTemplate を活用する。

                            // 簡易的な方法: テンプレートとして読み込む
                            loadTemplate({
                                name: 'AI Generated',
                                description: 'Generated by AI Tuner',
                                template: prompt,
                                variables: {} // 変数は自動抽出される
                            });
                        }}
                    />
                </div>
